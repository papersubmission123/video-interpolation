...
TRAIN_DIR_CLIPS = '/path/to/2Dshape/Clips/'
...
#SAVE_DIR = get_dir('../Save4_ucf/')
SAVE_DIR = get_dir('/path/to/Save_2dshape/')
...
HIST_LEN = 2
# the number of predict frames when training
PRED_LEN = 14
...
# encoder for both forward and reverse
SCALE_FMS_GE = [[3 * HIST_LEN/2, 128, 256],
               [3 * (HIST_LEN/2 + PRED_LEN/2), 128, 256],
               [3 * (HIST_LEN/2 + PRED_LEN/2), 128, 256, 512],
               [3 * (HIST_LEN/2 + PRED_LEN/2), 128, 256, 512]]
SCALE_FMS_GD = [[512, 256, 128, 3*PRED_LEN],
               [512, 256, 128, 3*PRED_LEN],
               [1024, 512, 256, 128, 3*PRED_LEN],
               [1024, 512, 256, 128, 3*PRED_LEN]]
# kernel sizes for each convolution of each scale network in the generator model
SCALE_KERNEL_SIZES_GE = [[3, 3],
                        [5, 3],
                        [5, 3, 3],
                        [7, 5, 5]]
SCALE_KERNEL_SIZES_GD = [[3, 3, 3],
                        [3, 3, 5],
                        [3, 3, 3, 5],
                        [5, 5, 5, 7]]
...
SCALE_CONV_FMS_D = [[3*PRED_LEN, 64],
                    [3*PRED_LEN, 64, 128, 128],
                    [3*PRED_LEN, 128, 256, 256],
                    [3*PRED_LEN, 128, 256, 512, 128]]
