...
class DiscriminatorModel:
    ...
    def build_feed_dict(self, input_frames, gt_output_frames, generator):
        ...
        for scale_num in xrange(self.num_scale_nets):
            ...
            scaled_gt_output_frames = np.empty([batch_size, scale_net.height, scale_net.width, 3*c.PRED_LEN])
            for i, img in enumerate(gt_output_frames):
                ...
                resized_frame = resize(sknorm_img, [scale_net.height, scale_net.width, 3*c.PRED_LEN])
    ...
    def train_step(self, batch, generator):
        ...
        gt_output_frames = batch[:, :, :, 3*(c.HIST_LEN/2):-3*(c.HIST_LEN/2)]
