...
def get_train_batch():
    ...
    for i in xrange(c.BATCH_SIZE):
        ...
        path = c.TRAIN_DIR_CLIPS  + str(np.random.choice(200000)) + '.npz'
...
'''
def get_test_batch(test_batch_size, idx,  num_rec_out=1):
    """
    Test UCF101 from mathieus
    """
    clips = np.empty([test_batch_size,
                      c.FULL_HEIGHT,
                      c.FULL_WIDTH,
                      (3 * (c.HIST_LEN + num_rec_out))])

    path = c.TEST_DIR + str(idx)
    file_list = ['pred_1.png', 'pred_2.png', 'pred_3.png', 'pred_4.png', 'target_1.png', 'target_2.png', 'target_3.png']
