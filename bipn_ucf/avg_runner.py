...
class AVGRunner:
    ...
    def test(self):
        ...
        """
        j = 0
        #for i in range(1, 3782, 10):  # test alll from mathieus
        for i in range(0, 28):  # test 28 examples from joo
            batch = get_test_batch(c.BATCH_SIZE, i, num_rec_out=self.num_test_rec)
            print(batch.shape)
            self.global_step = j
            j += 1
